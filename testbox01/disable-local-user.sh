...
usage() {
  ...
  echo '  -r Removes the home directory associated with the acccount(s).' >&2
...
# Loopt through all the usernames supplied as arguments
for USERNAME in "${@}"
do
  ...
  # Create an archive if requested do do so.
  if [[ "${ARCHIVE}" = 'true' ]]
  then
